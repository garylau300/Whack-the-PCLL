...
function parseSheetXml(xml, sharedStrings) {
  ...
  while ((cm = cellRe.exec(xml))) {
    // A self-closing <c .../> has no body, so it cannot carry a value --
    // it is a styled-but-empty cell. Google Sheets pads its xlsx export out
    // to the sheet's full declared grid with these: a real week tab has
    // 25,859 of them against 123 cells that actually hold anything. An
    // empty cell and an absent one are indistinguishable to cellText(), so
    // skip these before doing any per-cell work. maxRow/maxCol then
    // describe the populated region rather than the padded one, which is
    // what extractWeek's scan bounds actually want.
    if (cm[1] !== undefined) continue;
    const attrs = cm[2];
    const body = cm[3] || '';
...
// parseSheetXml is exported for scripts/verify-parser.js -- the padding-cell
// skip below is the whole reason the parse is fast, so it is worth pinning.
module.exports = { loadWorkbook, parseSheetXml };
