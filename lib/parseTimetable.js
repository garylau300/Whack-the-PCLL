...
// HKU's own sheet types "PCLL8104" for Civil Advocacy in two spots (weeks
// 12-13) where every other week — LG1-4, LG6, both SGs, the assessment, and
// the elective list itself — uses "PCLL8014". Same course throughout (same
// LG numbering, same instructor "NH"), so the typo'd code is normalized to
// the one used everywhere else rather than splitting one course in two.
const CODE_ALIASES = { PCLL8104: 'PCLL8014' };
function canonicalizeCode(code) {
  return code == null ? code : (CODE_ALIASES[code] || code);
}
...
// A "please see separate notice" SG session (no per-group Gp line at all,
// just a block of rooms reserved for the whole day) often wraps its room
// list across two grid cells — "CCT 310, ..., 323," / "523, 623, ..., 901"
// — with only the first line carrying the "CCT" prefix that VENUE_HINT_RE
// looks for. A follow-up line that's nothing but more comma-separated room
// numbers continues that same venue rather than starting a new topic line.
const ROOM_LIST_CONTINUATION_RE = /^\d{2,4}(?:\s*,\s*\d{2,4})*\s*,?$/;
// Likewise the trailing "(All rooms reserved from ...)" note (occasionally
// itself wrapped across two lines) belongs with the venue, not the topic.
const RESERVED_NOTE_RE = /\breserved\b/i;
...
function extractPreRecorded(grid, weekHeaderRow) {
  ...
  for (let r = labelRow + 1; r < weekHeaderRow; r++) {
    ...
    if (codeIdx !== -1) {
      ...
      const code = canonicalizeCode(COURSE_CODE_RE.exec(cells[codeIdx])[0].replace(/\s+/g, '').toUpperCase());
...
function classifyLines(lines, code) {
  ...
  // `code` is already canonicalized (see canonicalizeCode) — a raw line
  // typed with the un-normalized alias (e.g. "PCLL8104" for Civil Advocacy)
  // needs the same canonicalization before comparing, or it stops matching
  // bareCode and survives here as its own unstripped, phantom-topic line.
  ...
  const cleaned = lines
    ...
    .filter((l) => {
      ...
      const withoutFt = lineNoSpace.startsWith('FT') ? lineNoSpace.slice(2) : lineNoSpace;
      return canonicalizeCode(lineNoSpace) !== bareCode && canonicalizeCode(withoutFt) !== bareCode;
    });
...
function classifySegment(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (venue && (ROOM_LIST_CONTINUATION_RE.test(line) || RESERVED_NOTE_RE.test(line))) {
      venue = `${venue} ${line}`.trim();
      continue;
    }
...
function extractWeek(grid, maxRow, opts) {
  ...
  for (const day of days) {
    ...
    for (let r = bodyStart; r <= bodyEnd; r++) {
      ...
      if (block && block.lines.length) {
        ...
        if (codeMatch) {
          const newCode = canonicalizeCode(codeMatch[0].replace(/\s+/g, '').toUpperCase());
          const existingCode = block.lines
            .map((l) => COURSE_CODE_RE.exec(l))
            .find(Boolean);
          if (existingCode && canonicalizeCode(existingCode[0].replace(/\s+/g, '').toUpperCase()) !== newCode) {
            closeBlock();
          }
        }
      }
  ...
  // scope: 'all' on an SG session means no per-group Gp line was found at
  // all. That covers two very different situations: a whole-cohort LG
  // lecture (no "my group" concept to resolve — irrelevant here, never
  // SG-numbered) and an SG HKU scheduled without ever splitting it by group
  // — which itself splits into a resolvable case and an unresolvable one.
  // Civil Advocacy's SGs are a single Saturday slot the whole cohort
  // (including this student) attends together — one occurrence a week,
  // uncontested, safe to promote to 'group'. Criminal Advocacy's SG1-3 run
  // both Friday *and* Saturday under the same SG number with no letter to
  // tell them apart — two occurrences a week, genuinely ambiguous, left as
  // 'all' rather than guessing which day is really this student's.
  const weekEvents = days.flatMap((d) => d.events);
  const sgAllCounts = new Map();
  for (const ev of weekEvents) {
    if (ev.scope !== 'all' || !ev.no || !/^SG/i.test(ev.no)) continue;
    const key = `${ev.code}|${ev.no}`;
    sgAllCounts.set(key, (sgAllCounts.get(key) || 0) + 1);
  }
  for (const ev of weekEvents) {
    if (ev.scope !== 'all' || !ev.no || !/^SG/i.test(ev.no)) continue;
    if (sgAllCounts.get(`${ev.code}|${ev.no}`) === 1) ev.scope = 'group';
  }
...
function blockToEvents(block, grid, opts) {
  ...
  const code = codeMatch ? canonicalizeCode(COURSE_CODE_RE.exec(codeMatch)[0].replace(/\s+/g, '').toUpperCase()) : null;
...
function buildTimetable(workbook, opts) {
  ...
  for (const name of workbook.sheetNames) {
    ...
    // `courses` is metadata for the response envelope only -- nothing below
    // extractWeek reads it, so it isn't threaded through.
    const week = extractWeek(grid, maxRow, { group, pairedGroup });
    if (week) weeks.push(week);
  }
  // Chronological, which is what every consumer assumes (pickCurrentWeekIndex,
  // the dashboard's "next up", the week nav). The sheet currently numbers its
  // weeks 1-21 straight through both semesters, so semester is already implied
  // by the week number -- but a sheet that restarted at "Week 1" in January
  // would interleave the two years' worth of weeks under a week-only sort, and
  // nothing downstream would notice.
  weeks.sort((a, b) => (a.semester - b.semester) || (a.week - b.week));
  return {
    meta: {
      group,
      pairedGroup,
